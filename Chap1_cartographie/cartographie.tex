...
\noindent\fbox{\parbox{\linewidth-2\fboxrule-2\fboxsep}{\small La description des ESU est à l'heure actuel réalisé par plusieurs outils. Les plus répandus sont les modèles prédictifs de bruits basé sur des modèles de sources et de propagation. Ces modèles sont notamment utilisé pour la cartographie de bruit de trafic. Ces modèles possèdent des limites bloquantes. Une autre approches qui se développe est l'utilisation de mesures et d'enregistrements sonores en ville à travers des réseaux de capteurs fixes et des mesures mobiles. Cette approche permet de considérer l'ESU dans son intégralité. Toutefois l'une des limites à résoudre est l'extraction des sources sonores afin de connaitre la contribution de chacune des sources.}}
...
où $S_{int.}(t)$ résume les autres sources sonores, appelées sources \textit{interférantes}, qui n'appartiennent pas à la classe \textit{trafic}. 

Cette description peut également s'exprimer dans le domaine fréquentielle à l'aide d'une transformée de Fourier, tels que l'équation 

L'ESU $M_i(t)$ est alors un ensemble de sources variées, ayant une allure temporelle, fréquentielle ainsi qu'un niveau sonore propre, et qui sont émises dans un environnement où des phénomènes de propagation complexes ont lieu. Les questions soulevées sont alors :
