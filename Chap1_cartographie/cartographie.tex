...
Mises à jours tous les 5 ans, les cartes de bruit résument, pour chaque source de bruit, les niveaux sonores globaux moyens pondérés $A$ sur 24h ($L_{DEN}$ pour \textit{Day-Evening-Night} \ml{traduire ?}) et durant la nuit ($L_N$) :

\begin{equation}
L_{DEN} = 10\times\log \left(\frac{1}{24} \left(12\times10^{\frac{L_D}{10}}+4\times10^{\frac{L_E+5}{10}}+8\times10^{\frac{L_N+10}{10}} \right)\right)
\end{equation}

avec $L_D$, $L_E$ et $L_N$, les niveaux sonores moyens à long terme pondéré \ml{A ??} pour les périodes respectives 6h-18h, 18h-22h, 22h-6h (pouvant être changées suivant le rythme de vie des habitants de la zone considérée),
...
\ml{trop flou, tu dois parler de modèle prédictif basé physique et expliciter un peu}
...
\ml{l'information est la, mais je trouve la presentation un peu literaire, je formaliserai un peu plus en se basant sur le schema bloc pour donner une vue d'ensemble au debut, ce qui te permetra de parler de sig des le debut.}

\section{Limitations des modèles prédictifs de bruit de trafic}

\ml{il faut faire sentir des la présentation formelle le potentiel et les possibles insuffisances de l'approche}

\ml{on peut les re lister ici pour introduire la section}
...
Ensuite, le milieu urbain est considérablement simplifié pour éviter de complexifier et contrôler le temps de calcul nécessaire à la production de la prédiction. \ml{ca va pas dans le flux : L'influence des plus petits mobiliers urbains est également mise de coté.}


\subsection{Incertitudes des modèles physiques}

\ml{est ce que les modèles physiques peuvent etre incertains, je ne pense pas. Les méthodes les utilisant oui}
...
\ml{ca parle plus de l'hétérogénéité que d'incertitudes, a revoir}
...
\ml{sans une presentation d'au moins un modèle physique auparavant, c'est inaccessible}
...
L'utilisation de modèles dynamiques de trafic, couplés aux modèles d'émissions sonore  \cite{can2010traffic}, n'est actuellement pas destiné à la cartographie des \ml{ESU} mais à l'étude des interactions entre les véhicules et à leur cinématique.
...
\ml{il y a beaucoup de soucis d'accord, arrange toi pour faire relire le manuscrit une fois qu'il sera stabilisé par une personne compétente.}

\section{Vers la modélisation d'autres sources sonores ?}

De nombreuse études se sont intéressées à l'étude des ESU à travers la perception qu'en ont les citadins (\cite{brocolini_measurements_2013},  \cite{hong2013designing}). La plupart de ces études définissent des grandeurs acoustiques (\textit{activité}, la \textit{clarté, l'évolution temporelle, l'occupation spatiale}\dots) qui sont évaluées sur des échelles de taille variable limitées par des qualificatifs (respectivement \og monotone/varié \fg{}, \og brouhaha/distinct \fg{}, \og figée/évolutive \fg{}, \og peu présente/très présent \fg{} \dots) \cite{raimbault2003ambient}.
...
\ml{plus j'avance, et plus je me dis que le manuscrit a besoin d'une section "fondamentaux", avec un peu de physique et de perception, cela te permettra de poser pas mal de concepts clairement des le debut}
...
\ml{on voit se profiler différents usages, calibrage de modele predictifs, données pures, etc, a formaliser plus tot pour faire sentir pourquoi on a interet a lire tout ca.}
...
\ml{en finissant ici, on a l'impression que c'est ca qu'il faut faire, il convient a mon avis de faire un etat critique.}
