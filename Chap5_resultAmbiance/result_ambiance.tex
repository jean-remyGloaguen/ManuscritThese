...
Dans un premier temps, un estimateur de référence (ou \textit{baseline} en anglais) est nécessaire afin de comparer les performances de la NMF. La baseline choisie est un filtre passe-bas de fréquence de coupure $f_c$. L'hypothèse faite est que l'énergie située dans la bande passante est assimilable au signal \textit{trafic}, $\tilde{L}_{eq,tr.}$. Le choix de cet outil est justifié par la présence de composantes basses-fréquences (principalement en dessous de 1000 Hz) dans les signaux trafic. Supprimer l'énergie au-delà parait donc une première approche envisageable. %De plus, c'est une méthode qui pourrait être choisie par défaut.
...
%\ml{je viens de lire sur la mesure d'impact des éoliennes, et ils utilisent des termes comme niveau ambient, résiduel ou d'émergence, ce serait peut etre approprié des les utiliser.}
...
Là encore, la forme du dictionnaire reste la même et n'est pas influencée par le choix de la technique de seuillage. On constate que les valeurs de seuils $t_{f,1}$ et $t_{f,2}$ encadrent la valeur du seuil dur $t_h$. Si cet encadrement est large pour $\beta = 2$, pour $\beta \in \lbrace 0,~1 \rbrace$ cet encadrement est plus restreint. Cela correspond, pour $\beta$ = 1, à considérer dans $\mathbf{W}_{trafic}$, en moyenne 145 ($\pm$ 32) éléments issus directement de $\mathbf{W'}$ et à y inclure 8 ($\pm$ 2) éléments pondérés selon la relation \ref{eq:seuillageFirm_def}. Sur l'ensemble des cas, l'apport du seuillage \textit{firm} sur le erreurs $MAE_g$ est très faible. 
...
\begin{figure}[h!]
\centering
\subfigure[Influence du facteur expérimental $w_t$.]{%
\label{fig:influence_wt}%
\includegraphics[width=.45\linewidth]{./figures/resultats/influence_wt.pdf}}%
\qquad
\subfigure[Influence du facteur expérimental $K$.]{%
...
\caption{Erreurs $MAE$ pour chaque classe pour la NMF SUP \subref{fig:TIR_class_sup}, SEM \subref{fig:TIR_class_semi} et IS \subref{fig:TIR_class_IS}.}
\label{fig:TIR_bar}
\end{figure}
...
La valeur seuil $t_h$ = 0,41 est donc la valeur optimale permettant une erreur minimale sur l'intégralité du corpus, mais selon le sous-corpus ou la valeur du $TIR$ ce seuil est susceptible de varier.
