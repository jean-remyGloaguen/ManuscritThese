...
La constitution du dictionnaire est réalisée en trois étapes, résumées dans le diagramme en blocs en Figure \ref{fig:creation_W} : 
\begin{itemize}
...
\multirow{2}{*}{filtre PB} & 20 & - & - & - & 4,69 ($\pm$ 4,52) \\
 & 0,5 & - & - & - & 2,89 ($\pm$ 2,84) \\
 \midrule
\multirow{3}{*}{NMF SUP} & - & 0 & 0,5 & 50 & 4,36 ($\pm$ 4,16) \\
 & - & 1 & \textit{all} & 25 & 3,45 ($\pm$ 3,70) \\
 & - & 2 & 0,5 & 25 & 2,86 ($\pm$ 3,21)  \\
 \midrule
...
Puis, en Figure \ref{fig:influence_K} est résumé l'évolution des erreurs $MAE_g$ selon le nombre d'éléments $K$ contenu dans le dictionnaire.
Pour les 3 NMF, ce facteur est un paramètre plus influant que $w_t$. 
La NMF SUP privilégie un dictionnaire composé d'un faible nombre d'éléments. Ce comportement provient du fonctionnement de la NMF SUP en elle même : contrainte avec un seul dictionnaire fixe à s'adapter à l'ensemble des cas rencontrés, cette méthode privilégie les dictionnaires composé d'une quantité d'information réduite, plus générique afin d'être le plus généralisable que possible.
...
Dans le cas de la NMF IS cela se comprend d'autant qu'avec un nombre suffisant d'éléments dans le dictionnaire $\mathbf{W'}$, celui-ci permet une description plus fine et mieux adaptée à la scène. 
...
NMF SUP & 8,16 ($\pm$ 2,35) & 3,90 ($\pm$ 1,53) & 1,18 ($\pm$ 0,62) & \textbf{\textcolor{red}{0,31 ($\pm$ 0,18)}} & \textbf{0,74 ($\pm$ 0,07) } \\
