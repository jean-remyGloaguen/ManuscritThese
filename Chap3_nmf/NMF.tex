...
\noindent{\small \textbf{
Le fonctionnement de la NMF est présenté dans ce chapitre. Cette méthode consiste à approximer le spectrogramme en amplitude d'un signal audio par le produit de deux matrices positives : $\mathbf{W}$, un dictionnaire composés de spectres audio, et $\mathbf{H}$, la matrice d'activation. Les différents aspects de son fonctionnement sont décrit : famille de divergences, algorithmes de mise à jour des matrices, méthode d'apprentissage du dictionnaire. Une forme de NMF est également proposé par la NMF \textit{seuillée initialisée} où un dictionnaire appris sur la sources d'intérêt est mis à jour et dont les éléments relatifs à cette source sont ensuite sélectionné par une technique de seuillage. Enfin les différentes contraintes qui peuvent être apposé sur les matrices de la NMF sont évoquée notamment la contrainte de régularité temporelle et de parcimonie.}}
...
Afin de répondre au problème de généralisation de $\mathbf{W}$, une autre approche est proposée : la \textit{NMF initialisée seuillée} (NMF-IS). Un dictionnaire initial, $\mathbf{W_0}$, est appris sur la source sonore cible (le trafic routier), puis est mis à jour avec $\mathbf{H}$ lors de la phase de test.
...
\begin{numcases}{h_k^{(i+1)} =}
    h_k^{(i)}\left(\frac{\sum_f w_{fk} v_f \tilde{v}_f^{(\beta-2)}}{\sum_f w_{fk} \tilde{v_f}^{(\beta-1)}+\alpha_{sp}}\right)^{\gamma(\beta)}, & $\beta < 2$,\label{eq:h_sparsity_1}\\
    h_k^{(i)}\left(\frac{\sum_f w_{fk} v_f \tilde{v}_f^{(\beta-2)}-\alpha_{sp}}{\sum_f w_{fk} \tilde{v_f}^{(\beta-1)}}\right)^{\gamma(\beta)}, & $\beta \geq 2$.\label{eq:h_sparsity_2}
\end{numcases}
\end{subequations}

Dans le cas où $\beta \geq 2$, la contrainte apparait au numérateur et est soustractive. Ce comportement peut être problématique dans le cadre de la non-négativité suivant la value de $\alpha_{sp}$ \cite{fevotte_algorithms_2011}. En Figure \ref{fig:sparsity_example}, un exemple de l'effet de la parcimonie sur l'allure d'un élément $j$ de la matrice $\mathbf{H}$ pour 3 valeurs de parcimonie. Avec l'augmentation de sa valeur certaines activations présentes lorsque la contrainte est absente mais de faibles amplitudes deviennent nulles. 
...
En conclusion, la Factorisation en Matrices Non-négative est une méthode qui, dans le domaine de l'audio, permet l'approximation d'un spectrogramme en amplitude (ou en puissance) d'un audio par le produit de deux matrices : $\mathbf{W}$, le dictionnaire composé de spectres sonores, et $\mathbf{H}$, la matrice d'activation. Différentes formes de NMF ont été proposées qui diffèrent selon les techniques d'apprentissage de leur dictionnaire (non-supervisée, supervisée et semi-supervisée) ou des contraintes qui y sont appliquées (parcimonie, régularité temporelle). Plusieurs sont implémentés dans le cas de ces travaux et testés en vue de déduire, sur des corpus de sons présentés dans le chapitre suivant, le niveau sonore du trafic routier.
La \textbf{NMF supervisée}, \textbf{semi-supervisée} et, celle proposée, \textbf{initialisée-seuillée} sont les trois approches retenues. L'influence de la contrainte de \textbf{régularité temporelle} $C_t(\mathbf{H})$ et de \textbf{parcimonie} $C_{sp}(\mathbf{H})$ seront également observées pour un des corpus de sons dans le chapitre \ref{chap:grafic}. Les 3 $\beta$-divergences détaillées (\textbf{distance EUC} ($\beta = 2$), \textbf{divergence K-L} ($\beta = 1$) et \textbf{I-S} ($\beta = 0$)) seront celles utilisées sur chaque corpus.
