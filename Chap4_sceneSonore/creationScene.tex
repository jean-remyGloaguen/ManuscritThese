...
\caption{Zoom du spectrogramme (nombre de points $w = 2^{12}$ avec 50 $\%$ de recouvrement) dans la bande de fréquence $\left[1500-7500 \right]$ Hz d'un enregistrement de passage de véhicule (véhicule Renault, rapport 3, 40 km/h). \`A gauche, l'enregistrement original, à droite l'enregistrement filtré par le filtre médian.}
...
À partir de ce corpus constitué, disponible en ligne\footnote{\url{https://doi.org/10.5281/zenodo.1213793}}, il est possible de réaliser des corpus de scènes sonores urbaines. En vue d'étudier le comportement de la NMF puis d'en évaluer les performances, deux corpus de scènes sonores urbaines sont construits.
...
\begin{equation}
TIR = L_{eq,tr., tir, i} - L_{eq,int., i}
\end{equation}
...
En tout 750 scènes sont ainsi disponibles (6 sous-corpus $\times$ 25 scènes $\times$ 5 $TIR$) pour une durée totale du corpus de 6h30. Les scènes de ce corpus ne peuvent pas être assimilables à des enregistrements sonores réalisés en ville, mais permettront d'étudier le comportement des estimateurs dans le chapitre \ref{chap:ambiance}. Ce corpus de sons est disponible en ligne\footnote{\url{https://doi.org/10.5281/zenodo.1145855}}. 
...
Une majorité de scènes appartiennent à l'ambiance sonore \textit{rue calme} (35 scènes), 23 scènes appartiennent à l'ambiance \textit{rue bruyante}, 8 scènes à l'ambiance \textit{parc} et 8 scènes à l'ambiance \textit{rue très bruyante}. Plus de la moitié des points de mesures possèdent la même ambiance sur les 4 trajets. À l'exception du point 10, tous les points de mesures possèdent deux ambiances sonores voisines. Ces variations proviennent de l'évolution des activités dans la journée (matin ou l'après-midi). Enfin, les points 3 et 19 du parcours 1-WE ne sont pas exploitables : le point 3 est pollué par un camion balayeur et le point 19 n'a pas été correctement enregistré. Au final, c'est 74 fichiers audio qui sont disponibles et utilisés pour créer des scènes sonores. Ces 74 enregistrements forment le \textit{corpus de référence}.
...
Notons que dans \textit{Rue calme}, \textit{bruyante} et dans \textit{Parc}, le décompte des voitures est assez aisé. Il l'est beaucoup moins dans \textit{rue très bruyante} où un flot de véhicules peut être présent, le comptage y est alors très délicat car les véhicules peuvent être considérés à la fois comme bruit de fond et évènements sonore.
...
L'optimisation du plan ne permet également pas d'avoir un nombre de réplications $R$ constant sur l'ensemble des scènes testées mais variable évoluant dans l'intervalle $\left[20,30 \right]$ (Figure~\ref{fig:replication}). \\


\begin{figure}[ht]
\centering
\includegraphics[width = 0.7\textwidth]{./figures/test_perceptif/nb_replication.pdf}
\caption{Nombre de réplications, $R$, pour chaque scène obtenu dans $X_{opt}$ avec comme combinaison $J = 50$, $B = 40$, $K = 20$. Les 20 premières scènes sont les scènes issues des enregistrements du projet GRAFIC, les 20 suivantes sont les scènes répliquées sous \textit{SimScene}.}
...
\resizebox{\textwidth}{!}{%
\begin{tabular}{L{2cm}C{1cm}C{1cm}C{1cm}C{1cm}C{1cm}C{1cm}C{1cm}C{1cm}C{1cm}C{1cm}}
\toprule
...
\rowcolor[HTML]{C0C0C0}
\textbf{valeur} $\mathbf{t}$ & 0,29 & 0,39 & 1,15 & 0,30 & 0,84 & 0,32 & 0,82 & 2,20 & 0,24 & 0,45 \\
\textbf{valeur} $\mathbf{p}$ & \textbf{0,38} & \textbf{0,35} & \textbf{0,13} & \textbf{0,38} & \textbf{0,20} & \textbf{0,37} & \textbf{0,21} & 0,01 & \textbf{0,40} & \textbf{0,33}\\ \bottomrule
\end{tabular}}
\end{table}
...
\hhline{|-|-|~|-|-|~|-|-|~|-|-|}
Parc & {\cellcolor[HTML]{5AB25A}} & & Rue calme & {\cellcolor[HTML]{FFCB2F}} & & Rue bruyante & {\cellcolor[HTML]{F56B00}} & &  Rue très bruyante & {\cellcolor[HTML]{9A0000}}\\
\hhline{|-|-|~|-|-|~|-|-|~|-|-|}
\end{tabular}
