...
La méthode de la NMF a été retenue afin d'obtenir les niveaux sonores du trafic routier. Mais son utilisation directe sur des enregistrements sonores (où le niveau sonore du trafic réel est inconnu) ne permet pas de connaitre son efficacité. En effet, comment connaitre avec justesse le niveau sonore estimé puisque que le niveau sonore réel exacte est lui-même inconnu ? La solution proposée est ici d'appliquer cet estimateur sur des scènes sonores urbaines simulées où la contribution du trafic routier sera connue et où les estimations des niveaux sonores pourront être comparé aux solutions exactes. Plusieurs questions sont alors soulevées : comment composer des mixtures sonores urbaines aussi réalistes que des enregistrements sonores ? Comment s'assurer que les corpus de sons sur lesquels sont appliqués les algorithmes permettent de tester les limites des estimateurs ? 
Plus généralement, la création d'environnements sonores urbains dépasse celle de l'estimation du niveau sonore du trafic. 
L'intérêt d'utiliser des scènes sonores issus d'un processus de simulation est qu'il permet un meilleur contrôle sur leur composition où la présence des différentes classes de son, ainsi que leur niveau sonore, est alors défini par l'utilisateur en fonction de ses besoin.
Leur utilisation peut trouver un intérêt pour dans le cadre des études perceptives des ESU. Leur utilisation a déjà été réalisée pour, par exemple, étudier l'influence d'une source sonore sur l'évaluation de l'agrément sonore \cite{lafay_new_2014}, ce que ne permet pas l'écoute faites en villes \cite{adams_soundwalking_2008, liu2014effects} ou en laboratoire d'enregistrements audio \cite{guastavino2005ecological, cain2013development}. 
Ces scènes sonores simulées peuvent également trouver un intérêt pour tester et développer des outils de traitement du signal \cite{Komatsu2016, geiger2015improving} où l'utilisation de scènes sonores simulées permet d'éviter l'étape de l'annotation manuelle, qui peut être longue et fastidieuse.

Toutefois, si l'utilisation de scènes sonores simulées a de nombreux avantages, elle pose la question de leur validité écologique. En effet, la réalisation de \textit{soundwalks} ou l'utilisation d'enregistrements sonore permettent de baser ces études sur des scènes sonores réelles et donc de mieux s'assurer que les outils ou les lois créés sont bien adaptés à ces environnements.
Développé avec des scènes sonores simulées, si celles-ci ne sont pas construites avec certaines précautions, les lois et les outils qui y sont développés pourront ne plus être adaptés à des sons réels.
...
Ce chapitre relate ainsi la création des corpus de scènes sonores urbaines simulées. Dans une première partie, plusieurs approches, pour simuler de tels environnements, sont résumées, puis l'outil retenu est présenté en détail. Les étapes menant à la création d'une base de données, appelé corpus élémentaire, et de deux corpus d'évaluation sont ensuite exposées. Enfin, le test perceptif et les résultats obtenu, démontrant le réalisme d'un des corpus, sont présentés.
...
Les enregistrements des passages de voitures sont, quant à eux, séparés en deux parties : les enregistrements issus des deux premiers véhicules (Renault Mégane, Renault Clio) sont inclus dans le corpus élémentaire afin de construire les scènes sonores, les autres échantillons des deux autres voitures (Renault Sénic, Dacia Sandero) serviront dans la partie \ref{chap:NMF} afin de construire le dictionnaire de la NMF et ainsi éviter le surapprentissage.
...
Par soucis de concision, seul l'ensemble des 20 valeurs $t$ calculé et les boites à moustaches de chaque scène sont résumés dans les Figures  \ref{fig:test-student} et \ref{fig:boxplot_scene}.
