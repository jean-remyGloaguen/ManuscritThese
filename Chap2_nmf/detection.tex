...
Savoir isoler les composantes qui constituent un signal est une question vaste et complexe intervenant dans des domaines variés comme en biologie \cite{}, dans le monde médical \cite{} ou bien encore en télécommunication \cite{}. Dans le domaine de l'audio, cette question intervient alors que la quasi-totalité des sons qui nous entourent sont composé d'une multitude de sources sonores. Le problème se pose mathématiquement comme : 
...
Chacune de ces sources sonores possèdent ses propres caractéristiques acoustiques (intensité, durée, fréquences) et pouvant être simultanées, avoir entre elles des liens de causes à effets ou bien encore avoir été altérée lors de la propagation dans un milieu. 
Le développement de tels outils permet alors d'éditer, d'analyser ou de modifier les composantes des mixtures sonores, pour ainsi, par exemple : 

\begin{itemize}
\item débruiter la voix pour améliorer la qualité des appareils auditifs,
\item isoler des instruments de musique pour restaurer des vieux enregistrements de musique,
\item estimer la présence de sources sonores spécifiques comme les oiseaux dans un environnement.
\item \dots\\
\end{itemize}

De part la variété des sons, c'est donc une tâche complexe qui est à l'étude depuis plus de 30 ans. De ces recherches, plusieurs méthodes et approches ont émergés qui diffèrent selon les hypothèses faites sur les sources. On propose dans ce chapitre de présenter certaines des méthodes les plus couramment utilisées. 
...
La CASA est inspirée de l'Analyse de Scènes Auditives selon Bregman \cite{bregman1994auditory} qui explore les façons dont le cerveau humain comprend et organise les sons qui l'entoure. L'architecture de la CASA se décompose alors en 4 parties \cite{wang2006computational} : 
...
\item une corrélation croisée entre les canaux fréquentiels proches pour faire émerger la présence des formants, 
...
\item Un masquage binaire temps-fréquence qui construit et appliqué sur le spectrogramme initial isole les différentes sources sonores.\\
\end{itemize}

L'utilisation de cette méthodes à des sons environnementaux est très limitée. Des applications dans la reconnaissance de scènes sonores existent \cite{peltonen2002computational} mais sont peu nombreuses. Cette méthode se révèlent surtout apte à extraire des composantes harmoniques et a donc surtout été développée dans le cas des signaux de paroles \cite{ellis1999using, brown2005separation, shao2010computational}. 

\section{Algorithme DUET}

L'algorithme de séparation de sources DUET (\textit{Degenerate Unmixing Estimation Techniques)} est une méthode proposée par \cite{rickard2007duet} qui permet de déterminer $N$ sources sonores d'une mixture sonore à partir du déphasage et de l'atténuation entre les enregistrements de deux microphones $x_1(t)$ et $x_2(t)$. Cette approche se base sur 2 hypothèses : les signaux sont émis dans des conditions anéchoïques et il n'y a pas (ou très peu) de recouvrement fréquentiel entre les $N$ sources sonores $s_j(t)$. Les conditions d'anéchoïcité permettent d'exprimer les 2 signaux captés sous la forme d'un signal exprimé en champ direct, $x_1(t) = \sum_{j = 1}^{N}s_j(t)$, et d'un autre, $x_2(t) = \sum_{j = 1}^{N} a_j s_j(t-\delta_j)$, pondéré par l'atténuation $a_j$ et en déphasage (positif ou négatif) $\delta_j$ avec la condition $\delta_j \leqslant \frac{d_{\mu_1,\mu_2}}{c_0}$ où $d_{\mu_1,\mu_2}$ est la distance entre les microphones et $c_0$ la célérité du son. En raison de la distance entre les microphones et du déphasage entre les signaux, le dispositif limite la fréquence maximale du signal qu'il peut traiter à $f_{max} = \frac{c_0}{2 d_{\mu_1,\mu_2}}$. Le problème s'exprime sous forme matriciel : 
...
Une implémentation de cet algrorithme pour une utilisation en temps réel est proposée \cite{rickard2001real}.
...
Une large revue exhaustive des méthodes employées sont détaillées dans \cite{hyvarinen2004independent}.
Pour être résolu, l'ICA est, le plus souvent, sur-déterminé, ce qui signifie qu'il y a au moins autant de signaux captés que de sources sonores. Pour obtenir une matrice $\mathbf{A}$ carré, l'ACP peut être utilisée afin de supprimer les informations redondantes.  Le cas de la sous-détermination a toutefois été étudié, par exemple dans \cite{bofill2000blind} en considérant les signaux comme parcimonieux.
L'ACI a trouvé de nombreuses applications, en tant que méthode de séparation aveugle de sources, dans le domaine médical, pour extraire les différentes composantes d'un signal d'électroencéphalogramme \cite{delorme2007enhanced,makeig1996independent} ou d'une IMR \cite{lee1999independent}, pour traiter des signaux contenant de la parole \cite{sarela2005denoising, hsieh2009independent} ou pour des contenus musicaux \cite{uhle2003extraction, abdallah2003independent}. Des utilisations de l'ACI pour des sons environnementaux existent aussi \cite{lombard2011tdoa, eronen2006audio}. Cette méthode peut également être utilisée pour des antennes acoustiques et pour de la formation de voie (\textit{beamforming})\cite{cardoso_blind_1998,saruwatari2003blind}.

Séparer aveuglement les sources contenues dans leurs mélanges est entaché d’ambiguïté.
En effet, la séparation peut être obtenue avec une infinité de solutions [90],
[91]. Pour chaque cas, instantané ou convolutif, le comportement de l’ambiguïté a une
incidence sur la reconstruction des sources.

La détermination des matrices $\mathbf{A}$ et $\mathbf{s}$ ne sont toutefois pas sans générer des ambiguïtés bien identifiées. La première est l'\textit{ambiguïté de permutation} qui traduit la variabilité dans l'ordre de détermination des composantes indépendantes : une composante estimée $s_1$ peut être déterminé à un rang différent sans pour autant changer la reconstruction du signal global.  Ce problème est toutefois dans conséquence la plupart du temps. Une seconde limite est l'\textit{ambiguité d'échelle} qui traduit la possibilité d'avoir un facteur d'échelle présent dans la matrice $\mathbf{A}$ et $\mathbf{s}$ tel que : 
...
La Factorisation en Matrices Non-négatives (abrégé NMF pour \textit{Non-negative Matrix Factorization} en anglais) \cite{lee_learning_1999} est une méthode qui est basée sur la représentation linéaire de données : 
...
En conclusion, la méthode CASA et l'algorithme DUET sont des méthodes qui sont peu adapté au cas d'étude présent, car développé notamment pour des sons harmoniques et qui nécessité au minimum, deux microphones pour chaque point de mesure. Pour la même raison, l'ACI n'est pas une méthode adaptée à la mise en place de réseaux de capteurs monophoniques. De plus l'\textit{ambiguité d'échelle} est un frein pour estimer correctement le niveau sonore du trafic. L'outil DESA répond à une grande partie des besoins, mais, comparé à la NMF, ne gère pas aussi bien le problème du recouvrement temporel, où à un instant $t$, un évènement est classé soit \textit{trafic} soit \textit{anormal}. Or dans certain cas, le recouvrement peut être très important et s'étendre dans la durée. Cette approche serait susceptible de mettre de coté trop d'échantillons. C'est donc, la NMF qui, au vue de son fonctionnement (reconstruire le spectrogramme d'un signal à l'aide d'un dictionnaire), est la méthode la plus adaptée au cadre de cet étude.
