...
Savoir isoler les différentes composantes qui constituent un signal est une question vaste et complexe intervenant dans des domaines variés comme en biologie \cite{chiappetta2004blind}, dans le monde médical \cite{jung2000removing} ou bien encore dans le domaine de l'image \cite{}. En audio, cette question intervient alors que la quasi-totalité des sons qui nous entourent sont composées d'une multitude de sources sonores. En considérant, $N$ sources sonores, pour 1 capteur $i$, le problème de la séparation de source peut se poser comme un problème linéaire inverse : 
...
L'intérêt de développer des outils de séparation de sources sur de tels mixtures donne la possibilité d'éditer, d'analyser ou de modifier les composantes, avec comme applications, par exemple, 
...
De part la variété des sons, c'est donc une tâche complexe qui est à l'étude depuis plus de 30 ans. De ces recherches, plusieurs méthodes et approches ont émergés qui diffèrent selon les approches choisies. On propose dans ce chapitre de présenter certaines des méthodes les plus couramment utilisées pour déterminer celle qui est la plus adapté au cas d'étude de la thèse. 
...
Développée à partir de l'analyse des sons par notre cerveau, la méthode CASA a notamment trouvé des applications dans le domaine de la parole, en cela qu'elle est adaptée pour retrouver des sons harmoniques \cite{ellis1999using, brown2005separation, shao2010computational}. 
L'utilisation de cette méthode à des sons environnementaux est donc très limitée puisque ces sons sont susceptibles d'être plus bruités. Des applications dans la reconnaissance de scènes sonores existent \cite{peltonen2002computational} mais les applications de cette méthode à ce genre d'environnement sont peu nombreuses. 
...
La détermination des matrices $\mathbf{A}$ et $\mathbf{s}$ ne sont toutefois pas sans générer des ambiguïtés bien identifiées dans la littérature. La première est l'\textit{ambiguïté de permutation} qui traduit la variabilité dans l'ordre de détermination des composantes indépendantes : une composante estimée $s_1$ peut être déterminé à un rang différent sans pour autant changer la reconstruction du signal global.  Ce problème est toutefois dans conséquence la plupart du temps. Une seconde limite est l'\textit{ambiguité d'échelle} qui traduit la possibilité d'avoir un facteur d'échelle présent dans la matrice $\mathbf{A}$ et $\mathbf{s}$ tel que : 
